...
def integrateImage(imageWidth,imageHeight) :
    ...
    print ("Now converting pixel values into values of an integral image")
    print ("\n")

    pix2[0][0] = pow(pix2[0][0],2)
    print ("Before at [10][10]:", pix[10][10])
    ...
    print ("After at [10][10]:", pix[10][10])
    print ("\n")
...
def parseXml() :
    ...
    print ("Referring to haarcascade database")
...
def evalStages(windowX,windowY,windowWidth,windowHeight,listHaarCascade,scale) :
    ...
    #print ("Evaluating stages. This will take time")
    ...
    for stage in listHaarCascade:
        ...
        for tree in stage[:-1]:
            ...
            while True:
                ...
                featureSum,invArea,vnorm = evalFeature(windowX,windowY, windowWidth, windowHeight,rects,scale)
        ...
        if not stagePass :
            failcount+=1
            #print ("Stage Fail. Count:", failcount)

            return stagePass

    
    print ("Stage pass:", stagePass)
    return stagePass
        ...
        while windowX < imageHeight-scale*24:
            ...
            while windowY < imageWidth-scale*24:
                ...
                if evalStages(windowX,windowY,windowWidth,windowHeight,listHaarCascade,scale):
                    ...
                    print ("roi:", windowX,windowY,windowWidth,windowHeight)
                    print ("Inserting roi in listResult")
...
def simplifyRects(listResult):
    ...
    if len(listResult)>0:
        ...
        print ("Coordinates:", simplifiedList)
        print ("\n")
...
def drawRect(simplifiedList) :
    ...
    for rect in simplifiedList :
        ...
        draw.line((windowX, windowY, windowX, windowY+windowHeight), (0,0,255),lineWidth)

        draw.line((windowX, windowY+lineWidth/2, windowX+windowWidth, windowY+lineWidth/2), (0,0,255),lineWidth) # Adds half of width

        draw.line((windowX+windowWidth,windowY,windowX+windowWidth,windowY+windowHeight), (0,0,255),lineWidth)

        draw.line((windowX, windowY+windowHeight-lineWidth/2, windowX+windowWidth, windowY+windowHeight-lineWidth/2),(0,0,255),lineWidth)
...
pix  = [[int((30*pixels[y,x][0]+59*pixels[y,x][1]+11*pixels[y,x][2])/100) for x in range(imageWidth)] for y in range(imageHeight)]
...
cascade = etree.parse("haarcascade_frontalface_alt2.xml").getroot().find("haarcascade_frontalface_alt2")
